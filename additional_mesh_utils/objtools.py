...
def showObj(mesh_path, pitch=-30, yaw=30, roll =0, z=1, save_image = False, show_3d = True):
    """shows a 3d parametrable view of the object. has an option to save the view into a flat image file
    Args:
        mesh_path (_type_): path to the mesh
...
def showFolder(folder_path, pitch=-30, yaw=30, roll=0, z=1, save_image = False, show_size = False, show_3d = True):
    """show all the 3d meshes in the folder
    Args:
        folder_path (string): path to the folder containing the meshes
        show_size (bool, optional): shows each file's size. Defaults to False.
    """
    for file in listdir(folder_path):
        if file.endswith(".obj") or file.endswith(".stl"):
            # Montrer l'objet
            showObj(file, pitch, yaw, roll, z, save_image, show_3d)
            # Donner la taille de l'objet
            if show_size :
                show_size(path.join(folder_path, file))
